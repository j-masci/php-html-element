...
/**
 * The trait to be used in ElementBaseClass.
 *
 * An HTML element object contains a tag, attributes, and children.
 *
...
Trait T_ElementBaseClass{
    ...
    /**
     * todo: deep clone children as well ? debatable.
     */
    public function __clone(){
        foreach ( $this->atts as $name => $attr ) {
            $this->atts[$name] = clone $attr;
        }
    }
    ...
    public function attr_get_instance( $name ) {
        return isset( $this->atts[$name] ) ? $this->atts[$name] : $this->build_attribute_instance( $name );
    }
     ...
     * @return $this
     */
    public function _compound_attr_add( $name, ...$args ){
